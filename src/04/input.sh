...
if [ "$#" -ne 0 ]; then
  echo -e "\e[31mError! Please do not use any parameters. Parameters must be specified in the config file.\e[0m"
  exit 1
fi
if ! [[ $column1_background =~ ^[1-6]$ && $column1_font_color =~ ^[1-6]$ && $column2_background =~ ^[1-6]$ && $column2_font_color =~ ^[1-6]$  ]]; then
    echo -e "\e[31mError! Invalid parameters in config.txt. Please use a numbers from 1 to 6. Using default colors.\e[0m"
    echo
...
if [[ $column1_background == $column1_font_color || $column2_background == $column2_font_color ]]; then
  echo -e "\e[31mError! Font and background colors must not match. Using default colors.\e[0m"
  echo
  column1_background=$default_column1_background
  column1_font_color=$default_column1_font_color
  column2_background=$default_column2_background
